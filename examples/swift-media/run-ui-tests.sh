#!/usr/bin/env bash
#
# Run SwiftMedia XCUITests on the iOS Simulator.
#
# Usage:
#   ./run-ui-tests.sh
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}SwiftMedia UI Tests${NC}"
echo -e "${BLUE}============================================${NC}"
...
set +e
xcodebuild test \
    -project SwiftMedia.xcodeproj \
    -scheme SwiftMedia \
    -destination "id=$SIMULATOR" \
    -only-testing:SwiftMediaUITests \
    2>&1 | tee /tmp/swiftmedia-uitest-output.log \
...
if [ $TEST_EXIT -eq 0 ]; then
    ...
    echo "Full log: /tmp/swiftmedia-uitest-output.log"
fi
echo -e "${BLUE}============================================${NC}"
