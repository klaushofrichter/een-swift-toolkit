#!/usr/bin/env bash
#
# Run SwiftEvents XCUITests on the iOS Simulator.
#
# Usage:
#   ./run-ui-tests.sh
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}SwiftEvents UI Tests${NC}"
echo -e "${BLUE}============================================${NC}"
...
set +e
xcodebuild test \
    -project SwiftEvents.xcodeproj \
    -scheme SwiftEvents \
    -destination "id=$SIMULATOR" \
    -only-testing:SwiftEventsUITests \
    2>&1 | tee /tmp/swiftevents-uitest-output.log \
...
if [ $TEST_EXIT -eq 0 ]; then
    ...
    echo "Full log: /tmp/swiftevents-uitest-output.log"
fi
echo -e "${BLUE}============================================${NC}"
