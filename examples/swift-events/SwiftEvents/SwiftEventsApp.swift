...
struct SwiftEventsApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(toolkit: toolkit)
                ...
                .task {
                    if await injectFromEnvironment() { return }
                    if await injectTestCredentials() { return }
    ...
    private func injectFromEnvironment() async -> Bool {
        ...
        if restored {
            print("[SwiftEvents] Authenticated via environment variables")
        }
        return restored
    }
    ...
    private func injectTestCredentials() async -> Bool {
        ...
        let restored = await toolkit.restoreSession()
        if restored {
            print("[SwiftEvents] Authenticated via injected test credentials")
        }
        return restored
    }
